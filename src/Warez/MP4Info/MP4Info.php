...
namespace Warez\MP4Info;

use Warez\MP4Info\Box as Box;
...
class MP4Info {
	...
	public static function getInfo($file) {
		...
		// Get all boxes
		try {
			while (($box = Box\MP4Info_Box::fromStream($f))) {
				$boxes[] = $box;
			}
		} catch (Exception $e) { }
			...
			switch ($box->getBoxTypeStr()) {
				case 'hdlr':
					switch ($box->getHandlerType()) {
						case Box\MP4Info_Box_hdlr::HANDLER_VIDEO:
							$context->hasVideo = true;
							break;
						case Box\MP4Info_Box_hdlr::HANDLER_SOUND:
